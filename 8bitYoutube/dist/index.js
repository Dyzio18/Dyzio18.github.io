...
var _videoController = __webpack_require__(28);

var _commentsController = __webpack_require__(32);

var videoURL = 'https://www.youtube.com/watch?v=zcq4c0I_sVk';

var appView = exports.appView = function appView(root) {
    ...
    document.getElementById('searchButton').addEventListener("click", function () {
        ...
        if (searchContent) {
            (0, _videoController.getVideoData)(searchContent).then(function (data) {
                displayVideoData(data);
            }).catch(function (err) {
                error.log(err);
            });
            (0, _commentsController.getCommentsData)(searchContent).then(function (data) {
                console.log(data[1]);
...
var displayVideoData = function displayVideoData(response) {
...
var displayCommentsData = function displayCommentsData(response) {
...
var configYoutube = exports.configYoutube = {
...
exports.getVideoId = undefined;
exports.getVideoData = getVideoData;

var _requestServices = __webpack_require__(30);
...
function getVideoData(url) {
    ...
    var parseUrl = generateVideoUrl(url);
    try {
        return (0, _requestServices.sendRequest)(parseUrl).then(function (response, error) {
 ...
 */
var generateVideoUrl = function generateVideoUrl(url) {
    return 'https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&id=' + getVideoId(url) + '&maxResults%7D&key=' + _config.configYoutube.userKey;
};
 ...
 */
var getVideoId = exports.getVideoId = function getVideoId(url) {
    var idVideo = url.match(/(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)/);
    if (idVideo !== null) {
 ...
 * @return {object}
 */
function getCommentsData(url) {
    try {
        var parseUrl = generateCommentsUrl(url);
        return (0, _requestServices.sendRequest)(parseUrl).then(function (response, error) {
            var commentData = [];
 ...
 */
var generateCommentsUrl = function generateCommentsUrl(url) {
    return 'https://www.googleapis.com/youtube/v3/commentThreads?key=' + _config.configYoutube.userKey + '&textFormat=plainText&order=relevance&part=snippet&videoId=' + (0, _videoController.getVideoId)(url);
};
